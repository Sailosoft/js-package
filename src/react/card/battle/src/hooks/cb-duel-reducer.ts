...
export function cbDuelReducer(state: CBCardLayout, action: DuelAction): CBCardLayout {
  switch (action.type) {
    case 'DRAW_CARD': {
    ...
    case 'PLAY_CARD': {
      const cardIndex = state.hand.cards.findIndex(c => c.id === action.cardId);
    ...
    case 'DESTROY_CARD': {
      const slotIndex = state[action.field].findIndex(s => s.id === action.slotId);
      const card = state[action.field][slotIndex]?.card;

      if (!card) return state;

      const newField = [...state[action.field]];
      newField[slotIndex] = { ...newField[slotIndex], card: undefined, status: 'empty' };

      return {
        ...state,
        [action.field]: newField,
        graveyard: { ...state.graveyard, cards: [card, ...state.graveyard.cards] },
      };
    }
